...
export default {
  async fetchBookmarks(context: ActionContext<State, State>): Promise<void> {
    try {
      ...
      context.commit(types.SET_LOGGED_IN, {
        loggedIn: true,
      } as SetLoggedInPayload);
      localStorage.setItem(
        constants.LOCAL_STORAGE_BOOKMARKS,
        JSON.stringify(bookmarks)
      );
      return await Promise.resolve();
    } catch (e) {
      console.error(e);
      context.commit(types.SET_BOOKMARKS, {
        bookmarks: [],
      } as SetBookmarksPayload);
      context.commit(types.SET_LOGGED_IN, {
        loggedIn: false,
      } as SetLoggedInPayload);
      localStorage.setItem(
  ...
  selectLabel(
    ...
    { label }: SelectLabelActionPayload
  ) {
    context.commit(types.SET_SELECTED_LABEL, {
      selectedLabel: label,
    } as SetSelectedLabelPayload);
    context.commit(types.SET_CURRENT_PAGE, {
      currentPage: 0,
    } as SetCurrentPagePayload);
  ...
  setCurrentPage(
    context: ActionContext<State, State>,
    { page }: SetCurrentPageActionPayload
  ) {
    context.commit(types.SET_CURRENT_PAGE, {
      currentPage: page,
    } as SetCurrentPagePayload);
